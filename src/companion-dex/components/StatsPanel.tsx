...
import { Brain, Clock, Coffee, Cookie, Heart, Laugh, ListChecks, MessageCircle, Quote, Scale, Smile, Sparkles, TrendingUp, Users, Zap } from "lucide-react";
import { MOOD_LABEL } from "../data";
import type { Character, Theme } from "../types";
import { Avatar } from "./Avatar";
import { Bar, Block, Heatmap, Sparkline, Vital } from "./MetricSections";
  ...
  return (
    <div style={{ padding: "0 24px 8px" }}>
      ...
      <Block icon={<Heart size={15} />} title="Your bond">
        <Bar label="WARMTH" value={v(character.warmth)} />
        <Bar label="DEPTH" value={v(character.depth)} />
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16 }}>
          <span style={{ fontSize: 12, color: "var(--ink-2)", fontWeight: 600 }}>trend · 7 weeks</span>
          <span key={`sp-${character.id}`} className="cd-fade">
            <Sparkline data={character.trajectory} />
          </span>
        </div>
      </Block>
